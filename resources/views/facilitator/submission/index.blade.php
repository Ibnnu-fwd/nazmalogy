<x-app-layout>
    <x-breadcrumb :items="[
        ['text' => 'Dashboard', 'link' => route('facilitator.index')],
            ...
            <div class="flex flex-col md:flex-row items-center justify-between space-y-3 md:space-y-0 md:space-x-4 mb-4">
                ...
                <div
                    ...
                    <x-button text="Tambah Kategori" icon="add" backgroundColor="primary" hoverColor="primary"
                        fontSize="text-tiny" id="add-submission-button" onclick="add()"
                        modalTarget="create-submission" />
                </div>
            </div>
    ...
    @push('js-internal')
        <script>
            function edit(id) {
                ...
                let url = "{{ route('facilitator.submission.change-status', ':id') }}";
                url = url.replace(':id', id);
                $('#create-submission form').attr('action', url);
                $.ajax({
                    url: "{{ route('facilitator.submission.show', ':id') }}".replace(':id', id),
                    method: 'GET',
